...
import random
import copy
from AIFactoryDS.ImageUtilities import *


def generate_base_anchor_boxes(anchor_scale=[8, 16, 32], anchor_ratio=[1, 2, 1/2], image_size=(400, 400)):
    """
    :param anchor_scale:
    :param anchor_ratio:
...
def build_vgg16_base(input_tensor=None, trainable=False, include_output_layer=False, image_shape=(400, 400)):
...
def build_base_model(input_tensor=None, trainable=False, include_output_layer=False, image_shape=(400, 400), backbone='VGG16'):
...
def calc_rpn(annotation, image_width=400, image_height=400,
             anchor_box_scales=[8, 16, 32], anchor_box_ratios=[1, 2, 1/2], rpn_max_overlap=0.7, rpn_min_overlap=0.3):
    downscale = 16.0
...
def rpn_to_roi(rpn_layer, regr_layer, use_regr=True, max_boxes=300, overlap_thresh=0.9, std_scaling=4.0,
               anchor_box_scales=[8, 16, 32], anchor_box_ratios=[1, 2, 1/2], rpn_stride=16):
...
if __name__ == "__main__":
    ...
    rpn_position_model = Model(img_input, rpn_position)
